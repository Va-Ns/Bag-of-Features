...
ip_file_names =  genFileNames({Global.Interest_Dir_Name}, ...
    Categories.All_Train_Frames,"RUN_DIR\interest_points", ...
    Global.Interest_File_Name,'.mat',Global.Num_Zeros);

load("RUN_DIR\SIFT_features_of_interest_points\SIFT_features.mat");
get_train_indices = Categories.All_Train_Frames;
%% How many images are we processing?
nImages = length(Categories.All_Train_Frames);
descriptors = [];

for i = 1:length(get_train_indices)

    descriptors =[descriptors; features{get_train_indices(i)}];
...
% [centers,sse] = vgg_kmeans(double(descriptors'), codebook_size, ...
%     cluster_options);
[~,centers,sse] = kmeans(double(descriptors),300,"MaxIter",10, ...
...
save('RUN_DIR\Codebook\Codebook.mat', 'centers');
