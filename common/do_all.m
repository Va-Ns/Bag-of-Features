...
%% Platform requirements: The current implementation only runs under 32-bit Linux
...
%% Software requirements: Matlab Image Processing toolbox
...
if strcmp(EXPERIMENT_TYPE,'plsa')
    ...
    do_random_indices('config_file_1');
    
    %% copy & resize images into experiment subdir
    do_preprocessing('config_file_1');
    
    %% run interest operator over images
    do_interest_operator('config_file_1');
    
    %% obtain representation of interest points
    do_representation('config_file_1');
    
    %% form appearance codebook
    do_form_codebook('config_file_1','Codebook',1000);
