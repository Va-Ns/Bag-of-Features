...
%% Also rescales file containing ground truth locations of objects
...
mkdir RUN_DIR\resized_images\

%%%% Go through each of the categories 
for cat = 1 : Categories.Number
  
  %%% Generate filenames for images
  in_file_names = genFileNames(Categories.Name(cat), ...
      Categories.Frame_Range{cat},IMAGE_DIR,Global.Image_File_Name, ...
      Global.Image_Extension,Global.Num_Zeros);

  %%% load up ground truth location file (it exists)
  if exist(strjoin([IMAGE_DIR,'\',Categories.Name{cat},'\',Global.Ground_Truth_Name,'.mat'],''))
    %% load up file
    load(strjoin([IMAGE_DIR,'\',Categories.Name{cat},'\',Global.Ground_Truth_Name,'.mat'],''));
    %% copy the variable held in file
    gt_bounding_boxes_original = gt_bounding_boxes;
    clear gt_bounding_boxes; %% clear original
    %% set flag
    location_information_present = 1; % Flag για το αν έχουν φορτωθεί τα gt δεδομένα
  else
  ...
  for frame = 1:length(in_file_names)
    ...
    %%% read image in 
    im = imread(strjoin(in_file_names{frame},''));
    ...
    if location_information_present == 1 || location_information_present == 0 
                                      % αν τα gt δεδομένα, δεν κανονικοποιεί 
                                      % τα δεδομένα του background
      gt_bounding_boxes{frame} = gt_bounding_boxes_original{frame} * scale_factor;
    end
    
    %% Now save out to directory.
    fname = strjoin(["C:\Users\vasil\MATLAB Drive\Thesis\Matlab Projects" + ...
        "\bag_words_demo\common\RUN_DIR\resized_images",'\', ...
         Global.Image_File_Name, prefZeros(frame_counter, ...
         Global.Num_Zeros),'_resized',Global.Image_Extension],'');
    imwrite(im,fname);
  ...
  if location_information_present == 1 || location_information_present == 0 
    ...
    fn = strjoin(["C:\Users\vasil\MATLAB Drive\Thesis\Matlab Projects" + ...
        "\bag_words_demo\common\RUN_DIR",'/',Global.Ground_Truth_Name,'_', ...
        Categories.Name{cat},'_rescaled','.mat'],'');
