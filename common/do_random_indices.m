...
for a=1:length(Categories.Name) %% loop over each category...
  ...
  % Απλά προσθέτουν ένα offset +100 μπροστά από τους randperm δείκτες για
  % την δεύτερη κλάση.
  if (a>1)
    train_frames{a} = train_frames{a} + sum(cellfun('length', ...
        Categories.Frame_Range(1:a-1)));
    test_frames{a}  = test_frames{a}  + sum(cellfun('length', ...
        Categories.Frame_Range(1:a-1)));    
  end
...
save('RUN_DIR\random_indices.mat',"random_indices");
save('RUN_DIR\train_frames.mat',"train_frames");
save('RUN_DIR\test_frames.mat',"test_frames");
