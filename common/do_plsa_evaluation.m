...
%%% load up model
load("RUN_DIR\Model\Li.mat");
load("RUN_DIR\Model\Pd_z.mat");
load("RUN_DIR\Model\Pw_z.mat");
load("RUN_DIR\Model\Pz.mat");
load("RUN_DIR\Quantized_vector_descriptors\descriptors_vq.mat")
...
if ~exist('Pd_z_test') %%% only do this section the first time we look at the model
    ...
    test_indices = Categories.All_Test_Frames;
    test_interest_points = interest_points(test_indices);

    % ip_file_names =  genFileNames({Global.Interest_Dir_Name}, ...
    %     Categories.All_Test_Frames,RUN_DIR,Global.Interest_File_Name, ...
    %     '.mat',Global.Num_Zeros);

    %% Create matrix to hold word histograms from all images
    X = zeros(length(Categories.All_Test_Frames),VQ.Codebook_Size);
...
for i = 1:length(Categories.All_Test_Frames)
    ...
    [Pw_z,Pd_z_test,Pz_test,Li_test] = pLSA_EM(test_vq_descriptors', ...
        Pw_z,Learn.Num_Topics,Learn);
...
for a=1:nImage_Per_Figure:length(Categories.All_Test_Frames)
    ...
    for b=1:nImage_Per_Figure
        ...
        %%% load image
        im=imread(strjoin(image_file_names{index},''));
