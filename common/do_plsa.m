...
%     Categories.All_Train_Frames,RUN_DIR,Global.Interest_File_Name, ...
...
train_vq_descriptors = zeros(length(Categories.All_Train_Frames),size(centers,1));
...
% Δημιουργούμε τα training δεδομένα για τους δείκτες εκπαίδευσης που έχουμε
% ορίσει.
...
[Pw_z,Pd_z,Pz,Li] = pLSA_EM(train_vq_descriptors',[],Learn.Num_Topics,Learn);

%%% Now save model out to file
%[fname,model_ind] = get_new_model_name([RUN_DIR,'/',Global.Model_Dir_Name],Global.Num_Zeros);

save("RUN_DIR\Model\Pw_z.mat","Pw_z");
save("RUN_DIR\Model\Pd_z.mat","Pd_z");
save("RUN_DIR\Model\Pz.mat","Pz");
save("RUN_DIR\Model\Li.mat","Li");
