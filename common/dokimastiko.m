...
% με σκοπόπο την αυτόματη προσαρμογή του y άξονα.
grayresizedTrainds = transform(grayTrainds, ...
    @(x) imresize(x,XScale/size(x,2),'bilinear'));
grayresizedTestds = transform(grayTestds, ...
    @(y) imresize(y,XScale/size(y,2),'bilinear'));
...
getcurrentDirectory = pwd; EdgeSamplingLocation = uigetdir();
